...
pacman::p_load(tidyverse, metafor, patchwork, MuMIn, viridis, emmeans, broom, rphylopic)
...
mod0 <- rma.mv(yi, vi, method="ML", random = ~1|study_id, data=heatdome_ROM)
mod1 <- rma.mv(yi, vi, mods=~taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
mod2 <- rma.mv(yi, vi, mods=~response_type, method="ML", random = ~1|study_id, data=heatdome_ROM)

model.sel(mod1, mod2)

#model selection on fixed effects
#just taxonomic group 
moda <- rma.mv(yi, vi, mods = ~taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
#just impact
modb <- rma.mv(yi, vi, mods = ~timing, method="ML", random = ~1|study_id, data=heatdome_ROM)
# just movement
modc <- rma.mv(yi, vi, mods = ~movement, method="ML", random = ~1|study_id, data=heatdome_ROM)
# just response type 
modd <- rma.mv(yi, vi, mods = ~response_type, method="ML", random = ~1|study_id, data=heatdome_ROM)

#trying out different combos of fixed effects

#impact + taxonomic group
mode <- rma.mv(yi, vi, mods = ~timing + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
#impact + response type 
modf <- rma.mv(yi, vi, mods = ~timing + response_type, method="ML", random = ~1|study_id, data=heatdome_ROM)
#impact + movement
modg <- rma.mv(yi, vi, mods = ~timing + movement, method="ML", random = ~1|study_id, data=heatdome_ROM)
#response type + movement
modh <- rma.mv(yi, vi, mods = ~response_type + movement, method="ML", random = ~1|study_id, data=heatdome_ROM)
#response type + taxonomic group 
modi <- rma.mv(yi, vi, mods = ~response_type + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
#movement + taxonomic group
modj <- rma.mv(yi, vi, mods = ~movement + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)

#response type + impact + movement
modk <- rma.mv(yi, vi, mods = ~response_type + timing + movement, method="ML", random = ~1|study_id, data=heatdome_ROM)
#response type + impact + taxonomic group
modl <- rma.mv(yi, vi, mods = ~response_type + timing + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
#response type + movement + taxonomic group 
modm <- rma.mv(yi, vi, mods = ~response_type + movement + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)
#impact + movement + taxonomic group 
modn <- rma.mv(yi, vi, mods = ~timing + movement + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)

#response type + impact + movement + taxonomic group 
modo <- rma.mv(yi, vi, mods = ~ response_type + timing + movement + taxonomy, method="ML", random = ~1|study_id, data=heatdome_ROM)

model.sel(moda, modb, modc, modd, mode, modf, modg, modh, modi, modj, modk, modl, modm, modn, modo)
#modb, modg modf, are within 4ish AIC
#going with all of these


#model averaging the top models
eval(metafor:::.MuMIn)
final_model <- model.avg(model.sel(modb, modg, modf))
summary(final_model)
#generating CI's for the top model
confint(final_model, full = TRUE)


count(heatdome_ROM, study_id, movement, timing, response_type, taxonomy)



# Figure 3a -------------------------------------------------------------------

est <- as.data.frame(coef(final_model)) %>%
  rownames_to_column(var = "coefficient") %>%
  rename(est=`coef(final_model)`) %>%
  add_row(coefficient = "timingpost", est=-0.53566) %>%
  add_row(coefficient = "movementmotile", est=-0.53566) %>%
  add_row(coefficient = "response_typebehaviour", est=-0.53566) %>%
  add_row(coefficient = "overall", est= -0.9139)

ci <- as.data.frame(confint(final_model)) %>%
  rownames_to_column(var = "coefficient")  %>%
  rename(ci_lb= `2.5 %`, ci_ub = `97.5 %`) %>%
  add_row(coefficient = "timingpost", ci_lb=-1.2427408, ci_ub=0.17141032) %>%
  add_row(coefficient = "movementmotile", ci_lb=-1.2427408, ci_ub=0.17141032) %>%
  add_row(coefficient = "response_typebehaviour", ci_lb =-1.2427408, ci_ub=0.17141032) %>%
  add_row(coefficient = "overall", ci_lb = -1.3593, ci_ub=-0.4685)

final_model_df <- left_join(est, ci, by ="coefficient") %>%
  mutate(est_adj=case_when(coefficient %in% c("intrcpt", "timingpost", "movementmotile", 
                                              "response_typebehaviour", "overall") ~ est, 
                           coefficient %in% c("timingduring", "movementsessile", 
                                              "response_typeabundance/survival", "response_typedamage", 
                                              "response_typegrowth", "response_typereproduction") ~ est+-0.53566525), 
         ci_lb_adj = case_when(coefficient %in% c("intrcpt", "timingpost", "movementmotile", 
                                                  "response_typebehaviour", "overall") ~ ci_lb, 
                               coefficient %in% c("timingduring", "movementsessile", 
                                                  "response_typeabundance/survival", "response_typedamage", 
                                                  "response_typegrowth", "response_typereproduction") ~ ci_lb+-0.53566525), 
         ci_ub_adj = case_when(coefficient %in% c("intrcpt", "timingpost", "movementmotile", 
                                      "response_typebehaviour", "overall") ~ est, 
                   coefficient %in% c("timingduring", "movementsessile", 
                                      "response_typeabundance/survival", "response_typedamage", 
                                      "response_typegrowth", "response_typereproduction") ~ ci_ub+-0.53566525)) %>%
  rename(response=coefficient) %>% mutate(colour=case_when(response == "overall" ~ "red", 
                   TRUE ~ "black")) %>%
  filter(response != "intrcpt") %>%
  mutate(response = recode(response, timingpost = 'post', movementmotile = 'motile', response_typebehaviour =  'behaviour', 
                              overall = 'overall', timingduring = 'during', movementsessile = 'sessile', `response_typeabundance/survival` = 'abundance/survival', 
                              response_typedamage='damage', response_typereproduction = 'reproduction', response_typegrowth= 'growth'))
...
heatdome_responses <- heatdome_ROM %>%
  count(study_id, timing, movement, response_type, yi) %>%
  pivot_longer(cols=c(timing, movement, response_type), names_to = "variable", values_to="variable_level") %>%
  select(-n) %>%
  rbind(overall_resp) %>%
...
fig <-  heatdome_responses %>% 
  ...
  geom_linerange(data=final_model_df, aes(x=response, y=est, ymin=ci_lb, ymax=ci_ub, colour=colour), linewidth=2,
                position=position_dodge(.9))+
  geom_point(data=final_model_df, aes(x=response, y=est, fill=colour, colour=colour), shape=23, size=6)+
  ...
  coord_flip() +
  scale_x_discrete(limits = c("reproduction", "growth", "damage", "abundance/survival", 
                              "behaviour", "motile", "sessile", "post", "during", "overall"),
                   labels = c("Reproduction (n=2)", "Growth (n=2)", "Damage (n=2)", "Persistence (n=23)", 
                              "Behaviour (n=28)","Motile (n=34)", "Sessile (n=23)",
                              "After (n=36)", "During (n=21)", "Overall (n=57)")) +
  geom_vline(xintercept = 9.5, linetype = "dotted", alpha = 0.5) + 
  geom_vline(xintercept = 7.5, linetype = "dotted", alpha = 0.5) + 
  geom_vline(xintercept = 5.5, linetype = "dotted", alpha = 0.5) + 
  ...
  geom_text(data = NULL, x = 3, y = -4, label = "Response type", size=8, 
            colour="grey75", fontface="italic")+
  geom_text(data = NULL, x = 6.5, y = -4, label = "Movement", size=8, 
            colour="grey75", fontface="italic")+
  geom_text(data = NULL, x = 8.5, y = -4, label = "Heat Dome Timing", size=8, 
            colour="grey75", fontface="italic")
...
ROM <- heatdome_ROM %>%
  ...
  add_phylopic(uuid = "c127f813-1ae2-4948-8a8b-9f6324df8dac", x=15, y=-0.6, ysize=1.2, alpha=1,fill = "black")+ #Dombeya acutangula
  add_phylopic(uuid = "e4e306cd-73b6-4ca3-a08c-753a856f7f12", x=14, y=-1, ysize=1.3, alpha=1,fill = "black")+ #wolf
  add_phylopic(uuid = "8e61e166-11f4-4377-a923-9b5b597b6eba", x=13, y=-0.55, ysize=1.1, alpha=1,fill = "black")+ #snowshoe hare
  add_phylopic(uuid = "1a7e182f-b3a0-448b-8121-29d6bb70bc35", x=12, y=-0.75, ysize=1.7, alpha=1,fill = "black")+ #western wood pewee
  add_phylopic(uuid = "feaab8ab-b887-4875-801c-5c94c4b5da4b", x=11, y=-0.5, ysize=1.7, alpha=1,fill = "black")+ #barnacle
...
fig3<- fig / ROM /  a  + plot_layout(heights = c(2, 7, 1.2)) + 
  plot_annotation(tag_levels = 'a')


ggsave(filename = "./figures/fig3.png", plot =  fig3, dpi=700, width = 16, height = 16.5)
...
heatdome_ROM %>% 
  filter(species %in% c("Alces alces", "Canis lupus", "Catharus ustulatus", 
                        "Contopus cooperi", "Contopus sordidulus", "Odocoileus hemionus")) %>% 
  arrange(species) %>% 
  select(species, comparison_type, yi) %>% 
  View



#45% decrease for surfgrass?
